...
def _render_bar_subplot(ax, parts, vals_a, vals_b, model_a, model_b, title, ylabel, y_offset, shared_vals=None):
    """Render a single grouped bar subplot. Returns (bars_a, bars_b)."""
    ...
    ax.set_title(title, y=1.07, pad=0)
...
def plot_grouped_metric(
    ...
    _render_bar_subplot(ax, parts, vals_a, vals_b, model_a, model_b, title, ylabel, y_offset)
...
def plot_fpr_fnr(results_root, out_dir, model_a, model_b):
    ...
    _render_bar_subplot(
        ...
        title="FPR by NSC Part", ylabel="Rate (%)", y_offset=0.08,
        shared_vals=shared_vals,
    )
    _render_bar_subplot(
        axes[1], parts, fnr_a, fnr_b, model_a, model_b,
        title="FNR by NSC Part", ylabel="Rate (%)", y_offset=0.08,
        shared_vals=shared_vals,
    )

    out_path = Path(out_dir) / "fpr_fnr_side_by_side.png"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
